...
function findAll(arr, condition) {
  ...
  console.log('fcondition',condition)
  return arr.reduce((accum, current, currentIndex) => {
    console.log(accum,':',current,':',currentIndex)
    return condition(current) ? [...accum, currentIndex] : accum}, []
  )
}
...
const pares = findAll([1, 7, 2, 9], (x) => x % 2 === 0);
// [0, 1, 3], the positions of all odd elements
...
let result = [1, 2, 3, 4].reduceRight((x, y) => [x, y], [])
...
freq = [...'Mississippi'].reduce(
  (freq, c) => {
    console.log('f:',freq)
    return { ...freq, [c]: (c in freq ? freq[c] + 1 : 1) }},
  {}
)
console.log('freq:', freq) // { M: 1, i: 4, s: 4, p: 2 }
